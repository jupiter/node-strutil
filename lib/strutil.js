...
// "あい" => [ 0x3042,  0x3044 ]
exports.convertStringToUnicodeCodePoints = function convertStringToUnicodeCodePoints(str) {
  var surrogate_1st = 0;
  var unicode_codes = [];
...
exports.convertUtf8BytesToUnicodeCodePoints = function convertUtf8BytesToUnicodeCodePoints(utf8_bytes) {
  var unicode_codes = [];
  var unicode_code = 0;
  var num_followed = 0;
...
// Helper function.
exports.convertEscapedCodesToCodes = function convertEscapedCodesToCodes(str, prefix, base, num_bits) {
...
exports.convertEscapedUtf16CodesToUtf16Codes = function convertEscapedUtf16CodesToUtf16Codes(str) {
  return convertEscapedCodesToCodes(str, "\\u", 16, 16);
}

// r'\U00003042\U00003044' => [ 0x3042, 0x3044 ]
exports.convertEscapedUtf32CodesToUnicodeCodePoints = function convertEscapedUtf32CodesToUnicodeCodePoints(str) {
...
exports.convertEscapedBytesToBytes = function convertEscapedBytesToBytes(str, base) {
...
// "&amp;#12354;&amp;#12356;" => [ 0x3042, 0x3044 ]
// "&amp;#x3042;&amp;#x3044;" => [ 0x3042, 0x3044 ]
exports.convertNumRefToUnicodeCodePoints = function convertNumRefToUnicodeCodePoints(str, base) {
...
exports.convertUnicodeCodePointsToUtf16Codes = function convertUnicodeCodePointsToUtf16Codes(unicode_codes) {
  var utf16_codes = [];
...
exports.convertUnicodeCodePointToUtf8Bytes = function convertUnicodeCodePointToUtf8Bytes(unicode_code, base) {
  var utf8_bytes = [];
...
exports.convertUnicodeCodePointsToUtf8Bytes = function convertUnicodeCodePointsToUtf8Bytes(unicode_codes) {
  var utf8_bytes = [];
  for (var i = 0; i < unicode_codes.length; ++i) {
...
// 0xff => "ff"
// 0xff => "377"
exports.formatNumber = function formatNumber(number, base, num_digits) {
...
exports.encodeBase64Helper = function encodeBase64Helper(data) {
  var encoded = [];
  if (data.length == 1) {
...
exports.decodeBase64 = function decodeBase64(encoded) {
  var decoded_bytes = [];
  var data_bytes = [];
...
exports.encodeBase64 = function encodeBase64(data_bytes) {
  var encoded = '';
...
exports.decodeQuotedPrintableHelper = function decodeQuotedPrintableHelper(str, prefix) {
  var decoded_bytes = [];
...
exports.decodeQuotedPrintable = function decodeQuotedPrintable(str) {
...
exports.encodeQuotedPrintableHelper = function encodeQuotedPrintableHelper(data_bytes, prefix, should_escape) {
  var encoded = '';
...
exports.encodeQuotedPrintable = function encodeQuotedPrintable(data_bytes) {
...
// [ 0xE3, 0x81, 0x82, 0xE3, 0x81, 0x84 ] => "%E3%81%82%E3%81%84"
exports.encodeUrl = function encodeUrl(data_bytes) {
  var should_escape = function(b) {
...
// [ 0x3042, 0x3044 ] => "あい"
exports.convertUtf16CodesToString = function convertUtf16CodesToString(utf16_codes) {
  var unescaped = '';
...
// [ 0x3042, 0x3044 ] => "あい"
exports.convertUnicodeCodePointsToString = function convertUnicodeCodePointsToString(unicode_codes) {
  var utf16_codes = convertUnicodeCodePointsToUtf16Codes(unicode_codes);
  return convertUtf16CodesToString(utf16_codes);
}

exports.maybeInitMaps = function maybeInitMaps(encoded_maps, to_unicode_map, from_unicode_map) {
...
exports.maybeInitSjisMaps = function maybeInitSjisMaps() {
...
exports.maybeInitIso88591Maps = function maybeInitIso88591Maps() {
  maybeInitMaps(ISO88591_MAP_ENCODED, ISO88591_TO_UNICODE,
                UNICODE_TO_ISO88591);
}

exports.lookupMapWithDefault = function lookupMapWithDefault(map, key, default_value) {
  var value = map[key];
  if (!value) {
    value = default_value;
  }
...
exports.convertUnicodeCodePointsToSjisBytes = function convertUnicodeCodePointsToSjisBytes(unicode_codes) {
  maybeInitSjisMaps();
  var sjis_bytes = [];
  for (var i = 0; i < unicode_codes.length; ++i) {
    var unicode_code = unicode_codes[i];
...
exports.convertUnicodeCodePointsToEucJpBytes = function convertUnicodeCodePointsToEucJpBytes(unicode_codes) {
  maybeInitSjisMaps();
  var eucjp_bytes = [];
  for (var i = 0; i < unicode_codes.length; ++i) {
    var unicode_code = unicode_codes[i];
...
exports.convertUnicodeCodePointsToIso88591Bytes = function convertUnicodeCodePointsToIso88591Bytes(unicode_codes) {
...
exports.convertSjisBytesToUnicodeCodePoints = function convertSjisBytesToUnicodeCodePoints(sjis_bytes) {
  maybeInitSjisMaps();
  var unicode_codes = [];
...
exports.convertIso88591BytesToUnicodeCodePoints = function convertIso88591BytesToUnicodeCodePoints(latin_bytes) {
  maybeInitIso88591Maps();
  var unicode_codes = [];
...
// 0x2422 => 0x82a0
exports.convertJisX208CodeToSjisCode = function convertJisX208CodeToSjisCode(jis_code) {
  var j1 = jis_code >> 8;
...
// 0x82a0 => 0x2422
exports.convertSjisCodeToJisX208Code = function convertSjisCodeToJisX208Code(sjis_code) {
  var s1 = sjis_code >> 8;
...
exports.convertJisX208BytesToSjisBytes = function convertJisX208BytesToSjisBytes(jis_bytes) {
  var sjis_bytes = [];
...
exports.convertSjisBytesToJisX208Bytes = function convertSjisBytesToJisX208Bytes(sjis_bytes) {
  var jis_bytes = [];
...
exports.convertIso2022JpBytesToUnicodeCodePoints = function convertIso2022JpBytesToUnicodeCodePoints(iso2022jp_bytes) {
  maybeInitSjisMaps();
...
exports.convertEucJpBytesToUnicodeCodePoints = function convertEucJpBytesToUnicodeCodePoints(eucjp_bytes) {
  maybeInitSjisMaps();
  var unicode_codes = [];
...
exports.convertUnicodeCodePointsToIso2022JpBytes = function convertUnicodeCodePointsToIso2022JpBytes(unicode_codes) {
  maybeInitSjisMaps();
  var mode = ASCII;
...
// "=?UTF-8?B?44GC?=" => true
// "foobar" => false
exports.isMimeEncodedString = function isMimeEncodedString(str) {
...
// "INVALID" => []
exports.decodeMime = function decodeMime(str) {
...
exports.convertUnicodeCodePointsToBytes = function convertUnicodeCodePointsToBytes(unicode_codes, encoding) {
...
exports.convertBytesToUnicodeCodePoints = function convertBytesToUnicodeCodePoints(data_bytes, encoding) {
  var normalized_encoding = normalizeEncodingName(encoding);
...
// 'あい' => r'\u3042\u3044'
exports.escapeToUtf16 = function escapeToUtf16(str) {
  var escaped = ''
  for (var i = 0; i < str.length; ++i) {
...
exports.escapeToUtf32 = function escapeToUtf32(str) {
  var escaped = ''
...
// "あい" => "&#12354;&#12356;"
// "あい" => "&#x3042;&#x3044;"
exports.escapeToNumRef = function escapeToNumRef(str, base) {
  var unicode_codes = convertStringToUnicodeCodePoints(str);
  var escaped = ''
...
// "あい" => "l8je"
exports.escapeToPunyCode = function escapeToPunyCode(str) {
...
exports.convertBytesToEscapedString = function convertBytesToEscapedString(data_bytes, base) {
  var escaped = '';
  for (var i = 0; i < data_bytes.length; ++i) {
    var prefix = (base == 16 ? "\\x" : "\\");
...
exports.convertStringToPunyCodes = function convertStringToPunyCodes(str) {
  var unicode_codes = convertStringToUnicodeCodePoints(str);
  var puny_codes = [];
  var result = "";
...
// Requires: punycode.js should be loaded.
exports.convertPunyCodesToString = function convertPunyCodesToString(puny_codes) {
  var unicode_codes = [];
...
exports.escapeToEscapedBytes = function escapeToEscapedBytes(str, base, encoding) {
  var unicode_codes = convertStringToUnicodeCodePoints(str);
...
// "あい" => "44GC44GE"  // UTF-8
exports.escapeToBase64 = function escapeToBase64(str, encoding) {
  var unicode_codes = convertStringToUnicodeCodePoints(str);
  var data_bytes = convertUnicodeCodePointsToBytes(unicode_codes, encoding);
...
exports.escapeToQuotedPrintable = function escapeToQuotedPrintable(str, encoding) {
  var unicode_codes = convertStringToUnicodeCodePoints(str);
  var data_bytes = convertUnicodeCodePointsToBytes(unicode_codes, encoding);
...
exports.escapeToUrl = function escapeToUrl(str, encoding) {
  var unicode_codes = convertStringToUnicodeCodePoints(str);
  var data_bytes = convertUnicodeCodePointsToBytes(unicode_codes, encoding);
...
exports.escapeToMime = function escapeToMime(str, mime_encoding, char_encoding) {
  var unicode_codes = convertStringToUnicodeCodePoints(str);
...
// r'\u3042\u3044 => "あい"
exports.unescapeFromUtf16 = function unescapeFromUtf16(str) {
  var utf16_codes = convertEscapedUtf16CodesToUtf16Codes(str);
  return convertUtf16CodesToString(utf16_codes);
}

// r'\U00003042\U00003044 => "あい"
exports.unescapeFromUtf32 = function unescapeFromUtf32(str) {
...
exports.unescapeFromEscapedBytes = function unescapeFromEscapedBytes(str, base, encoding) {
...
// "&#12354;&#12356;" => "あい"
// "&#x3042;&#x3044;" => "あい"
exports.unescapeFromNumRef = function unescapeFromNumRef(str, base) {
  var unicode_codes = convertNumRefToUnicodeCodePoints(str, base);
  return convertUnicodeCodePointsToString(unicode_codes);
}

// "l8je" => "あい"
exports.unescapeFromPunyCode = function unescapeFromPunyCode(str) {
  var unicode_codes = convertStringToUnicodeCodePoints(str);
  return convertPunyCodesToString(unicode_codes);
}

// "44GC44GE" => "あい"
exports.unescapeFromBase64 = function unescapeFromBase64(str, encoding) {
...
exports.unescapeFromQuotedPrintable = function unescapeFromQuotedPrintable(str, encoding) {
...
// "%E3%81%82%E3%81%84" => "あい"
exports.unescapeFromUrl = function unescapeFromUrl(str, encoding) {
...
// " " => true
// " \n" => true
exports.isEmptyOrSequenceOfWhiteSpaces = function isEmptyOrSequenceOfWhiteSpaces(str) {
  for (var i = 0; i < str.length; ++i) {
...
exports.splitMimeString = function splitMimeString(str) {
  var parts = [];
  var current = "";
  while (str != "") {
...
// "UTF-8" => "UTF8"
// "Shift_JIS" => "SHIFTJIS"
exports.normalizeEncodingName = function normalizeEncodingName(encoding) {
...
exports.unescapeFromMime = function unescapeFromMime(str) {
